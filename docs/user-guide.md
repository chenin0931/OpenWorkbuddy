...
- 新工作和追问输入框的“添加文件”左侧提供任务级执行权限：**请求批准**保留当前工作区边界并在外部或高风险动作前暂停，**完全访问（自动）**把授权根扩展到整个磁盘，并自动执行普通读取、公开网页 GET/搜索、可逆写入与已分类的常规本地命令。选择会随工作保存，应用重启后不会悄悄改变。
...
- 完全访问仍会在删除、发送、发布、支付、上传、表单提交、凭据访问或未知外部副作用前暂停；高风险授权只能批准一次。参数校验、文件快照、陈旧写入保护、审计和非幂等动作不自动重放仍然有效。
- 文件写入前校验读取时的 SHA-256/mtime，并保存修改前快照；工作区外的变更同样生成 Diff 并支持带过期校验的撤销。
- `pwd`、`ls`、`rg`、只读 Git 查询等明确只读命令可自动执行。
- 产品不再提供“谨慎 / 平衡 / 高效”三级策略。设置页可以保存新工作的默认权限，每项工作仍可在输入框中独立切换，当前选择会随工作持久化并用于后续追问。
- 请求批准模式下，搜索、发送、发布、支付、上传、删除、网络命令、未知外部 MCP 操作和高风险不可逆动作会要求确认或被拒绝；完全访问模式只放宽普通操作，不放宽上述高风险边界。
