...
- 新工作和追问输入框的“添加文件”左侧提供任务级执行权限：**请求批准**保留当前工作区边界并在外部或高风险动作前暂停，**完全访问（自动）**把授权根扩展到整个磁盘，并自动执行未被策略硬拒绝的文件、Shell、网络、MCP 与浏览器操作。选择会随工作保存，应用重启后不会悄悄改变。
...
- 完全访问不再为未知 Shell、网络、删除或外部副作用逐项弹审批；参数校验、文件快照、陈旧写入保护、审计和非幂等动作不自动重放仍然有效。
...
- 请求批准模式下，搜索、发送、发布、支付、上传、删除、网络命令、未知外部 MCP 操作和高风险不可逆动作会要求确认或被拒绝；完全访问模式下，除确定性硬拒绝外会自动执行。
