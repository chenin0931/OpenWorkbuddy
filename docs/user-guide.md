...
- 文件读取仅限授权工作区；写入前校验读取时的 SHA-256/mtime，并保存修改前快照。
- `pwd`、`ls`、`rg`、只读 Git 查询等明确只读命令可自动执行。
- “设置 → 权限级别”提供三种低风险自动化程度：**谨慎**只自动只读动作；**平衡（默认）**再自动执行公开搜索、带快照与过期检查的工作区写入，以及单条测试、lint、类型检查或构建命令；**高效**再允许其他不联网、可逆的本地命令自动执行。
- 权限级别只影响低风险动作。发送、发布、支付、上传、删除、网络命令、未知外部 MCP 操作和高风险不可逆动作始终需要确认或被拒绝。
