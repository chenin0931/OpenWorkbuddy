...
test('首次启动完成模型、工作区和执行边界配置', async ({ page }) => {
  ...
  await expect(onboarding).toContainText('在输入框“添加文件”左侧选择“请求批准”或“完全访问（自动）”')
...
test('Kimi 设置支持目录、品牌样式，并在失败与取消后清空假 Key', async ({ page }) => {
  ...
  await expect(page.getByText('开始工作后仍可在“添加文件”左侧临时切换。')).toBeVisible()
...
test('工作台支持确认、添加文件与持久化 Diff 预览', async ({ page }) => {
  ...
  await expect(page.locator('.process-trigger')).toContainText('需要确认')
  ...
  await expect(inspector.getByRole('tab', { name: '诊断' })).toHaveCount(0)
...
test('新工作与追问可在添加文件左侧选择文件访问权限，并随请求持久化', async ({ page }) => {
  ...
  const runningAccess = page.getByLabel('工作执行权限')
  ...
  const newRunAccess = page.getByLabel('工作执行权限')
...
test('层级 Trace 只在二级诊断中按需展开，不暴露隐藏推理', async ({ page }) => {
  await installMockBridge(page, false, false, true)
  await page.goto('/')

  await expect(page.getByText('准备工具回执与来源')).toBeHidden()
  const inspector = await ensureInspector(page)
  await inspector.getByRole('tab', { name: '诊断' }).click()
...
test('新追问立即显示为右侧气泡，并排在执行过程之后且持久化后不重复', async ({ page }) => {
  await installMockBridge(page, false, false, false, true)
  await page.goto('/')

  const processTrigger = page.locator('.process-trigger')
  const collapsedHeight = await processTrigger.evaluate((element) => element.getBoundingClientRect().height)
  expect(collapsedHeight).toBeLessThanOrEqual(36)
  await processTrigger.click()
  const processSheet = page.getByRole('dialog', { name: '执行过程' })
  await expect(processSheet).toBeVisible()
  await expect(processSheet.getByRole('list', { name: '执行步骤' })).toBeVisible()
  await expect(processSheet).not.toContainText('tool-workspace')
  const handle = processSheet.getByRole('button', { name: '调整执行过程面板高度' })
  const heightBeforeResize = await processSheet.evaluate((element) => element.getBoundingClientRect().height)
  await handle.focus()
  await page.keyboard.press('ArrowUp')
  await expect.poll(() => processSheet.evaluate((element) => element.getBoundingClientRect().height)).toBeGreaterThan(heightBeforeResize)
  await page.screenshot({ path: `${FRONTEND_SCREENSHOT_DIR}/process-sheet-1440x900.png`, animations: 'disabled', fullPage: false })
  await page.keyboard.press('Escape')
  await expect(processSheet).toBeHidden()
  ...
  const placement = await page.evaluate((message) => {
    ...
    const process = document.querySelector<HTMLElement>('.process-trigger')
    if (!userMessage || !userBubble || !agentBubble || !timeline || !process) return undefined
    ...
    return {
      ...
      processBeforeUser: Boolean(process.compareDocumentPosition(userMessage) & Node.DOCUMENT_POSITION_FOLLOWING),
    }
  }, content)
  expect(placement).toBeDefined()
  expect(placement!.userX).toBeGreaterThan(placement!.agentX + 100)
  expect(placement!.rightGap).toBeLessThan(70)
  expect(placement!.processBeforeUser).toBe(true)
...
test('同一用户轮次的多段回复与多次工具调用合并为一个 WorkBuddy 响应', async ({ page }) => {
  ...
  await expect(firstTurn).not.toContainText('好的，我来搜索一下今天的新闻。')
  ...
  const firstTurnProcess = firstTurn.locator('.process-trigger')
  await expect(firstTurnProcess).toHaveCount(1)
  await expect(firstTurnProcess).toContainText('4 个步骤')
  const collapsedHeight = await firstTurnProcess.evaluate((element) => element.getBoundingClientRect().height)
  expect(collapsedHeight).toBeLessThanOrEqual(36)

  const firstTurnOrder = await firstTurn.evaluate((turn) => {
    const textNodes = [...turn.querySelectorAll<HTMLElement>('.markdown-content')]
    const process = turn.querySelector<HTMLElement>('.process-trigger')
    const result = textNodes.find((element) => element.textContent?.includes('第一次检索没有得到足够结果'))
    if (!process || !result) return undefined
    return {
      processBeforeResult: Boolean(process.compareDocumentPosition(result) & Node.DOCUMENT_POSITION_FOLLOWING),
    }
  })
  expect(firstTurnOrder).toEqual({ processBeforeResult: true })
...
test('普通已结束对话不显示完成庆祝、状态圆点或无证据的验证结论', async ({ page }) => {
  ...
  await expect(page.locator('.process-trigger')).toHaveCount(0)
...
test('多尺寸、浅深色工作台视觉基准', async ({ browser }) => {
  for (const { width, height } of [
    ...
    for (const theme of ['light', 'dark'] as const) {
      ...
      const process = page.locator('.process-trigger').first()
      const processHeight = await process.evaluate((element) => element.getBoundingClientRect().height)
      expect(processHeight).toBeLessThanOrEqual(36)
...
test('高对比度与 Reduce Motion 工作台视觉基准', async ({ browser }) => {
  for (const mode of ['high-contrast', 'reduce-motion'] as const) {
    ...
    await expect(page.locator('.agent-turn')).toHaveCount(2)
    const process = page.locator('.process-trigger').first()
    const height = await process.evaluate((element) => element.getBoundingClientRect().height)
