...
test('新追问立即显示为右侧气泡，并排在执行过程之后且持久化后不重复', async ({ page }) => {
  ...
  const processPanel = page.locator('.process-inline-panel')
  await expect(processPanel).toBeVisible()
  await expect(processPanel.getByRole('list', { name: '执行步骤' })).toBeVisible()
  await expect(processPanel).not.toContainText('tool-workspace')
  await expect(page.getByRole('dialog', { name: '执行过程' })).toHaveCount(0)
  await expect(page.locator('.process-sheet-backdrop')).toHaveCount(0)
  await page.screenshot({ path: `${FRONTEND_SCREENSHOT_DIR}/process-inline-1440x900.png`, animations: 'disabled', fullPage: false })
  await processTrigger.click()
