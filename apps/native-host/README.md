# OpenWorkbuddy Native Messaging Host
...
2. `~/Library/Application Support/OpenWorkbuddy/chrome.sock`.
...
The install script copies the binary into `~/Library/Application Support/OpenWorkbuddy/NativeHost` and writes Chrome's user-level manifest to `~/Library/Application Support/Google/Chrome/NativeMessagingHosts/com.onmyworkbuddy.chrome.json`. The manifest allowlists exactly the supplied extension ID.
